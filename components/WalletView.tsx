...
const WalletView: React.FC<WalletViewProps> = ({ user, orders, appConfig }) => {
  ...
  return (
    <div className={`p-4 pb-32 animate-in fade-in duration-500 min-h-screen rtl ${appConfig.backgroundImageUrl ? 'bg-transparent' : 'bg-white'}`} dir="rtl">
      <div className="mb-4">
         <h2 className="text-xl font-black text-white text-right">المحفظة (USD)</h2>
      </div>
      ...
      <div className="space-y-4">
        <h3 className="font-black text-white text-right text-sm px-2">العمليات الأخيرة</h3>
        {orders.length > 0 ? (
          orders.slice(0, 10).map(order => (
            <div key={order.id} className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-4 flex items-center justify-between">
               <div className="text-left">
                  <p className={`font-black text-xs ${order.type === 'recharge' ? 'text-emerald-400' : 'text-rose-400'}`}>
               ...
               <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <p className="text-[9px] text-white/40 font-bold">{order.date}</p>
                  </div>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${order.type === 'recharge' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                    {order.type === 'recharge' ? '+' : '-'}
                  </div>
               </div>
            </div>
          ...
          <div className="flex flex-col items-center justify-center py-10 opacity-60">
            <p className="font-bold text-white/40">لا توجد عمليات حالية</p>
          </div>
        )}
      </div>
    </div>
